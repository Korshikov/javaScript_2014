...
----
* В выражениях могут быть умножение ``*``, деление ``/``, сложение ``+``, вычитание ``-``, унарный минус, целочисленные константы, круглые скобки, переменные (для сложного -- ``x``, ``y`` и ``z``, для простого -- ``x``) и произвольное число пробельных символов в любом месте (не внутри констант).
